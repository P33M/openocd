...
#define NTAP_OPT_DP_ID 8
#define NTAP_OPT_INSTANCE_ID 9
...
static int jim_newtap_md_param(struct jim_nvp *n, struct jim_getopt_info *goi, struct jtag_tap *pTap)
{
	jim_wide w;
	int e = Jim_GetOpt_Wide(goi, &w);
	if (e != JIM_OK) {
...
static int jim_newtap_cmd(struct jim_getopt_info *goi)
{
	...
	tap->dotted_name = cp;
	tap->multidrop_targetsel = DP_TARGETSEL_INVALID;
	...
	if (!transport_is_jtag()) {
		...
		while (goi->argc) {
			...
			switch (n->value) {
				case NTAP_OPT_DP_ID:
					...
					e = jim_newtap_md_param(n, goi, tap);
					break;
				case NTAP_OPT_INSTANCE_ID:
		...
		if (instance_id_specified != target_id_specified) {
			LOG_ERROR("%s: -dp-id and -instance-id must both be specified", tap->dotted_name);
			free(cp);
			free(tap);
			return JIM_ERR;
		}
